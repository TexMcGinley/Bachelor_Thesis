...
class GameSession:
    ...
    def __init__(self, user_profile, epsilon_greedy, all_movies, connection, epsilon_game_session=None):
        '''
        Initializes the GameSession with user profile, epsilon greedy instance, movie list, and database connection.

        Args:
        ----
        - user_profile (UserProfile): The user's profile.
        - epsilon_greedy (EpsilonGreedy): The epsilon greedy algorithm instance.
        - all_movies (list): List of all available movies.
        - connection (sqlite3.Connection): Database connection.
        - epsilon_game_session (GameSession, optional): Epsilon greedy game session. Defaults to None.
        '''
    ...
    def add_to_recommendation_list_algorithm(self, movie, temp_score=None, position=None):
        '''
        Adds a movie to the recommendation list based on the epsilon greedy algorithm.

        Args:
        ----
        - movie (Movie): The movie to add.
        - temp_score (int, optional): Temporary score before adding the movie. Defaults to None.
        - position (int, optional): Position to insert the movie. Defaults to None.
        '''
        ...
        if movie.movie_id in available_movies_ids:
            ...
            self.update_score()
            score = self.score - temp_score if temp_score is not None else self.score
            self.algorithm_update_score(movie, score)
        else:
    ...
    def add_movie_by_algorithm(self):
        ...
        if selected_movie is not None:
            self.add_to_recommendation_list_algorithm(selected_movie, temp_score)
    ...
    def calculate_epsilon_greedy_score(self):
        '''
        Calculates the epsilon greedy score for the current round.
    ...
    def submit_round(self):
        '''
        Submits the current round and calculates the epsilon greedy score.

        Returns:
        ----
        - tuple: The user's score, epsilon greedy score, and epsilon value.
        '''
    ...
    def remove_from_recommendation_list(self, movie_id):
        '''
        Removes a movie from the recommendation list and updates the score.
    ...
    def update_score(self):
        '''
        Updates the total score of the recommendation list.
        '''
        self.score = calculate_total_score(self.user_profile, self.recommendation_list)

    def algorithm_update_score(self, movie, score):
        '''
        Updates the score for a movie using the epsilon greedy algorithm.

        Args:
        ----
        - movie (Movie): The movie to update the score for.
    ...
    def move_movie(self, from_index, to_index):
        '''
        Moves a movie within the recommendation list or between the recommendation list and available movies.

        Args:
        ----
        - from_index (int or str): The index or identifier of the movie to move.
        - to_index (int or str): The target index or identifier for the movie.
        '''
        from_ranked = False
        to_ranked = False
        ...
        if from_ranked and to_ranked:
            if (self.recommendation_list[from_index] is not None and self.recommendation_list[to_index] is not None):
                ...
                self.update_score()
                return
        elif (not from_ranked) and to_ranked:
            ...
            self.update_score()
            return
        elif from_ranked and not to_ranked:
            if from_index >= 0 and from_index < self.MAX_RECOMMENDATIONS:
                ...
                self.update_score()
                return
        elif not from_ranked and not to_ranked:
            from_movie = self.get_available_movie_by_id(from_index)
    ...
    def clear_recommendation_list(self):
        '''
        Clears the recommendation list and resets the score.
        '''
    ...
    def add_movie_at_rank(self, movie_title, rank):
        '''
        Adds a movie to the recommendation list at a specific rank.

        Args:
        ----
        - movie_title (str): The title of the movie to add.
        - rank (int): The rank position to insert the movie.
        '''
    ...
    def swap_movies(self, movie1, movie2):
        '''
        Swaps two movies within the recommendation list or available movies.

        Args:
        ----
        - movie1 (Movie): The first movie to swap.
        - movie2 (Movie): The second movie to swap.
        '''
        ...
        if movie1.title in recommendation_titles and movie2.title in recommendation_titles:
        ...
        elif movie1.title not in recommendation_titles and movie2.title in recommendation_titles:
        ...
        elif movie1.title in recommendation_titles and movie2.title not in recommendation_titles:
        ...
        elif movie1.title not in recommendation_titles and movie2.title not in recommendation_titles:
    ...
    def add_random_movie(self):
        '''
        Adds a random movie to the recommendation list.
        '''
        idx = 0
        for temp_movie in self.recommendation_list:
            if temp_movie is not None:
                idx += 1
        if idx < self.MAX_RECOMMENDATIONS:
            ...
            self.recommendation_list[temp_idx] = movie
    ...
    def add_to_recommendation_list(self, movie, rank=None):
        '''
        Adds a movie to the recommendation list at a specified rank or to the next available slot.

        Args:
        ----
        - movie (Movie): The movie to add.
        - rank (int, optional): The rank position to insert the movie. Defaults to None.
        '''
        if rank is None or self.recommendation_list[rank] is None:
            idx = 0
            for temp_movie in self.recommendation_list:
                if temp_movie is not None:
                    idx += 1
            ...
            answer = input(f"Do you want to swap {self.recommendation_list[rank].title} with {movie.title} ?(Y/N)")
            if answer.lower() == "y":
                self.available_movies.append(self.recommendation_list[rank])
                self.recommendation_list[rank] = movie
                self.available_movies.remove(movie)
    ...
    def get_available_movie(self, movie_title):
        '''
        Gets a movie from the available movies list by title.

        Args:
        ----
        - movie_title (str): The title of the movie to find.

        Returns:
        ----
        - Movie: The found movie, or None if not found.
        '''
    ...
    def get_available_movie_by_id(self, movie_id):
        '''
        Gets a movie from the available movies list by movie ID.
    ...
    def get_ranked_movie(self, movie_title):
        '''
        Gets a movie from the recommendation list by title.
    ...
    def display_recommendation_list(self):
        '''
        Displays the current recommendation list.
        '''
    ...
    def display_10_available_movies(self):
        '''
        Displays the first 10 available movies.
        '''
    ...
    def set_watched_movies(self, watched_movies):
        '''
        Sets the watched movies for the user profile.

        Args:
        ----
        - watched_movies (list of Movie): The list of watched movies.
        '''
    ...
    def fetch_watched_movies(self):
        '''
        Fetches the watched movies from the user profile.

        Returns:
        ----
        - list of Movie: The list of watched movies.
        '''
...
def create_game_session(user_profile, epsilon_greedy, all_movies, connection):
    '''
    Creates a game session for the user.

    Args:
    ----
    - user_profile (UserProfile): The profile of the user.
    - epsilon_greedy (EpsilonGreedy): The epsilon greedy algorithm instance.
...
def create_game_session_for_plots(user_profile, epsilon_greedy, all_movies, connection):
    '''
    Creates a game session for generating plots.
...
### Code for non Terminal version of game ### 
# def start_game1(epsilon_user, user_profile, epsilon_greedy, all_movies, connection):
#     '''
#     Starts the movie recommendation game.

#     Args:
#     ----
#     - epsilon_user (UserProfile): The profile of the epsilon user.
#     - user_profile (UserProfile): The profile of the user.
#     - epsilon_greedy (EpsilonGreedy): The epsilon greedy algorithm instance.
#     - all_movies (list): List of all movies available.
#     - connection (sqlite3.Connection): Database connection.
#     '''
#     epsilon_values = [0.99, 0.85, 0.6, 0.3, 0.001]
