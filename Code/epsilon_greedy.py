import random

class EpsilonGreedy:
    def __init__(self, epsilon, connection):
        '''
        Initializes the EpsilonGreedy instance with the specified epsilon value and database connection.

        Args:
        ----
        - epsilon (float): The exploration rate for the epsilon-greedy algorithm.
        - connection: The database connection object.
        '''
        self.epsilon = epsilon  # exploration rate
        self.connection = connection
        self.min_epsilon = 0.001  # Minimum value for epsilon
        self.movie_scores = {}  # Keep track of average scores for each movie

    def update_epsilon(self, epsilon):
        '''
        Updates the exploration rate (epsilon) to a specific value.

        Args:
        ----
        - epsilon (float): The new value for epsilon.
        '''
        self.epsilon = epsilon

    def explore_or_exploit(self, available_movies):
        '''
        Determines whether to explore or exploit based on the current epsilon value.

        Args:
        ----
        - available_movies (list): List of available movie objects.

        Returns:
        ----
        - Movie: The selected movie object.
        '''
        if random.random() < self.epsilon:
            return self.explore_random_movie(available_movies)
        else:
            return self.exploit_movie(available_movies)

    def explore_random_movie(self, available_movies):
        '''
        Randomly selects a movie from the available movies.

        Args:
        ----
        - available_movies (list): List of available movie objects.

        Returns:
        ----
        - Movie: The randomly selected movie object.
        '''
        best_movie = random.choice(available_movies)  # Pick a random movie
        return best_movie

    def exploit_movie(self, available_movies):
        '''
        Selects the best movie based on the average scores.

        Args:
        ----
        - available_movies (list): List of available movie objects.

        Returns:
        ----
        - Movie: The movie object with the highest average score.
        '''
        best_movie = max(
            available_movies,
            key=lambda m: self.movie_scores.get(m.movie_id, (0, 0))[0]  # Get the score part of the (score, count) tuple
        )
        return best_movie

    def get_movie_score(self, movie):
        '''
        Retrieves the average score for the specified movie.

        Args:
        ----
        - movie (Movie): The movie object.

        Returns:
        ----
        - float: The average score of the movie.
        '''
        return self.movie_scores.get(movie.movie_id, (0, 0))[0]

    def update_score(self, movie, score):
        '''
        Updates the average score for the specified movie.

        Args:
        ----
        - movie (Movie): The movie object.
        - score (float): The new score to incorporate into the average.
        '''
        if movie.movie_id in self.movie_scores:
            old_score, count = self.movie_scores[movie.movie_id]
            new_score = (old_score * count + score) / (count + 1)
            self.movie_scores[movie.movie_id] = (new_score, count + 1)
        else:
            self.movie_scores[movie.movie_id] = (score, 1)
